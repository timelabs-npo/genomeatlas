...
def verify():
    ...
    assert data["states"]["scientific"] == ["NA", "FAILED", "U", "C", "P", "0"]
    ...
    for file in (ROOT / "docs").rglob("*"):
        if file.is_file():
            assert not re.search(r"C:\\\\?Users|/home/|Bearer\s+[A-Za-z0-9]|genomeatlas-fresh-", file.read_text(encoding="utf-8")), file.name
    return 10
