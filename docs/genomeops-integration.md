...
The coordinator retains the original GenomeOps repository history on the private `timelabs-npo/ai-traces` `dedends` branch. The 35-ref bundle is verified and its SHA-256, restore test and retention metadata are recorded beside the bundle. The old `serg-alexv/genomeops-atlas` repository was retired only after that retention push and is no longer a second public source. No Git history is rewritten: the original SHAs remain recoverable in `ai-traces`, while `timelabs-npo/genomeatlas` is the single public source and deployment repository. See [RFC 0003](rfcs/0003-repository-consolidation.md), [RFC 0001](rfcs/0001-workbench-architecture.md), and [RFC 0002](rfcs/0002-scientific-evidence-contract.md).
