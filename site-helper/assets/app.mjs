...
function overview() {
  ...
  main.append(pageHeader('01','A clear path from data to evidence.','The frozen LAB panel, scientific workflow and tool inventory in one review workspace. Inspect the chain, check what is known, and prepare the next bounded request.'),
    ...
    el('div', {class:'split'},
      el('section', {}, sectionHead('What the snapshot establishes'),
        el('ul', {class:'compact-list'},
          ...
          el('li', {}, el('strong', {}, 'smarts.bio discovery requires authentication'), el('small', {}, 'AUTH_REQUIRED · a discovery attempt is not an analysis job.')))),
    ...
    callout('Native Sites tools are exposed in this session. Authentication and execution are NOT_TESTED; nothing is deployed. The original registry is an earlier snapshot. See Probe receipts for the separate discovery record.'),
